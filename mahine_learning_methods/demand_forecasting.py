...
def create_date_features(dataframe):
    ...
    dataframe["is_wknd"] = (dataframe.date.dt.weekday > 4).astype(int)
...
df.groupby(["store", "item", "month"]).agg({"sales":["sum", "mean", "median", "std"]}).head(20)

########################
# Random Noise
########################
# Aşırı öğrenmenin önüne geçmek için dataframe boyutunda rasgele gürültü oluşturuluyor.
...
def roll_mean_features(dataframe, windows):
    for window in windows:
        dataframe['sales_roll_mean_' + str(window)] = dataframe.groupby(["store", "item"])['sales']. \
                                                          transform(
            lambda x: x.shift(1).rolling(window=window, min_periods=10).mean()) + random_noise(
            dataframe)
    return dataframe
...
########################
# Bağımlı değişken sayısal olduğundan dolayı ve LightGBM gibi gradient descent temelli bir model kullanacağımızdan,
# iterasyon işlemlerinin süresi tahmin edilen değerlerle artıkların etkileşimine dayalı olduğundan dolayı standartlaştırmaya yönelik olacak.
# Zorunluluk değil, beklediğimiz şey train süresinin kısalması.
...
importance_zero = feat_imp[feat_imp["gain"] > 0.09]["feature"].values

...
from xgboost.spark.utils import deserialize_xgb_model
...
train.plot(title = "Single Exponential Smooting")
test.plot()
y_pred.plot()
plt.show()

train["1985":].plot(title = "Single Exponential Smooting")
test.plot()
y_pred.plot()
plt.show()
...
def plot_co2(train, test, y_pred, title):
    ...
    fig, ax = plt.subplots(figsize=(6, 4))  # Şablon oluştur

    train["1985":].plot(ax=ax, label="TRAIN", title=f"{title}, MAE: {round(mae, 2)}")
    test.plot(ax=ax, label="TEST")
    y_pred.plot(ax=ax, label="PREDICTION")

    ax.legend()  # Manuel legend ekleme
    plt.show()
